// modals.jsx — grounding, breathing, calming sounds, safety resources page
const { useState: useStateM, useEffect: useEffectM, useRef: useRefM, useCallback: useCallbackM } = React;

// ── Function 3: 5-4-3-2-1 grounding exercise ────────────────────────────
...
function GroundingExercise({ onComplete, onSkip }) {
  const [step, setStep] = useStateM(0);
  const overlayRef = useRefM(null);
  useEffectM(() => { overlayRef.current?.focus(); }, []);
  ...
  return (
    <div
      ref={overlayRef} tabIndex={-1} onKeyDown={onKey}
      className="fade-in"
      style={{
        position: 'absolute', inset: 0, zIndex: 80, outline: 'none',
        display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
        padding: '64px 28px', background: 'color-mix(in oklab, var(--bg) 90%, transparent)',
        backdropFilter: 'blur(14px)', WebkitBackdropFilter: 'blur(14px)',
      }}
      role="dialog" aria-modal="true" aria-label={`Grounding exercise: ${s.title}`}
    >
      <div role="status" aria-live="polite" className="sr-only" style={{ position: 'absolute', width: 1, height: 1, overflow: 'hidden', clip: 'rect(0 0 0 0)' }}>
        {`Step ${step + 1} of 5: ${s.title}`}
      </div>
      ...
      <div style={{ position: 'absolute', top: 26, left: 28, right: 28, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <button onClick={onSkip} aria-label="Skip grounding exercise"
          style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 13, fontWeight: 600, color: 'var(--ink-faint)' }}>
          Skip <Icon name="x" size={15} />
        </button>
      </div>

      <div className="scale-in" key={step} style={{ width: '100%', maxWidth: 440, textAlign: 'center' }}>
        <div style={{
          width: 92, height: 92, borderRadius: 26, margin: '0 auto 22px',
          display: 'grid', placeItems: 'center', background: 'var(--sage-soft)',
          color: 'var(--sage-deep)', boxShadow: 'var(--shadow-soft)',
        }}>
        ...
        <div style={{ display: 'flex', gap: 7, justifyContent: 'center', marginBottom: 14 }}>
          {Array.from({ length: s.count }).map((_, i) => (
            <span key={i} style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--sage)' }} />
          ))}
        </div>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 18 }}>
          {GROUNDING_STEPS.map((_, i) => (
            <span key={i} style={{
              ...
              background: i <= step ? 'var(--sage)' : 'var(--surface-3)',
              transition: 'background .3s var(--ease)',
            }} />
          ))}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <button onClick={back} disabled={step === 0} aria-label="Previous step"
            style={{
              display: 'inline-flex', alignItems: 'center', gap: 6, padding: '11px 18px',
              borderRadius: 'var(--r-pill)', fontWeight: 600, fontSize: 14,
          ...
          <span style={{ flex: 1, textAlign: 'center', fontSize: 12.5, fontWeight: 600, color: 'var(--ink-faint)' }}>
            Step {step + 1} of 5
          </span>
          <button onClick={next} aria-label={isLast ? 'Finish' : 'Next step'}
            style={{
              display: 'inline-flex', alignItems: 'center', gap: 7, padding: '11px 22px',
              borderRadius: 'var(--r-pill)', fontWeight: 700, fontSize: 14,
              background: 'var(--sage)', color: '#fff', boxShadow: 'var(--shadow-soft)',
            }}>
...
// ── Function 4: breathing / settle-body flow ────────────────────────────
const BREATH_PHASES = [
...
function BreathingModal({ onComplete }) {
  ...
  const timeoutRef = useRefM(null);
  const phaseRef = useRefM(0);
  ...
  useEffectM(() => {
    ...
    timeoutRef.current = setTimeout(() => {
      setPhase((i) => (i + 1) % BREATH_PHASES.length);
    }, p.dur);
  ...
  return (
    <div
      ...
      style={{
        ...
        display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
        padding: '64px 28px', background: 'color-mix(in oklab, var(--bg) 92%, transparent)',
    ...
    >
      <div style={{ position: 'absolute', top: 26, left: 28, right: 28, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--ink-faint)', fontVariantNumeric: 'tabular-nums' }} aria-live="off">{mm}:{ss}</span>
      </div>
      ...
      <div style={{ position: 'relative', width: 260, height: 260, display: 'grid', placeItems: 'center', marginBottom: 30 }}>
        <div aria-hidden="true" style={{
          ...
          background: 'radial-gradient(circle at 38% 32%, color-mix(in oklab, var(--sage) 24%, transparent), color-mix(in oklab, var(--sage) 6%, transparent))',
          border: '1px solid color-mix(in oklab, var(--sage) 30%, transparent)',
          transform: `scale(${scale})`, transition: `transform ${curDur}ms ${running ? (p.key === 'hold' ? 'linear' : 'cubic-bezier(.4,0,.4,1)') : 'var(--ease)'}`,
        }} />
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <button onClick={toggle}
          style={{
            ...
            borderRadius: 'var(--r-pill)', fontWeight: 700, fontSize: 14.5,
            background: 'var(--sage)', color: '#fff', boxShadow: 'var(--shadow-soft)',
          }}
          aria-label={running ? 'Pause breathing' : 'Start breathing'}>
          <Icon name={running ? 'pause' : 'play'} size={17} strokeWidth={2} fill="currentColor" />
          {running ? 'Pause' : 'Start'}
        </button>
        <button onClick={onComplete}
          style={{
            display: 'inline-flex', alignItems: 'center', gap: 7, padding: '13px 22px',
            borderRadius: 'var(--r-pill)', fontWeight: 600, fontSize: 14,
...
// ── Function 5: calming sounds (floating card, coexists with overlays) ──
const SOUND_TRACKS = [
...
function CalmingSounds({ onClose }) {
  const [selected, setSelected] = useStateM(null);
  const [playing, setPlaying] = useStateM(false);
  const [volume, setVolume] = useStateM(70);
  const [unavailable, setUnavailable] = useStateM(false);
  const audioRef = useRefM(null);
  ...
  useEffectM(() => { if (audioRef.current) audioRef.current.volume = volume / 100; }, [volume]);
  ...
  function close() { audioRef.current?.pause(); setPlaying(false); onClose(); }

  return (
    <div
      className="rise card"
      role="dialog" aria-label="Calming sounds"
      style={{ position: 'absolute', bottom: 20, right: 20, zIndex: 90, width: 280, padding: 0, overflow: 'hidden', boxShadow: 'var(--shadow-lift)' }}
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '13px 16px', borderBottom: '1px solid var(--line)' }}>
        ...
        <button onClick={close} aria-label="Close calming sounds" style={{ color: 'var(--ink-faint)', display: 'grid', placeItems: 'center', padding: 2 }}>
          <Icon name="x" size={16} />
        </button>
      </div>
        ...
        {SOUND_TRACKS.map((t) => {
          ...
          return (
            <button key={t.id} onClick={() => selectTrack(t.id)}
              aria-label={`${active && playing ? 'Pause' : 'Play'} ${t.label} sounds`} aria-pressed={active}
              style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '4px 16px 16px' }}>
        <button onClick={togglePlay} disabled={!selected}
          aria-label={playing ? 'Pause' : 'Play'}
          style={{
            width: 34, height: 34, borderRadius: '50%', flexShrink: 0, display: 'grid', placeItems: 'center',
            background: !selected ? 'var(--surface-3)' : 'var(--sage)', color: !selected ? 'var(--ink-faint)' : '#fff',
          }}>
          <Icon name={playing ? 'pause' : 'play'} size={15} strokeWidth={2} fill="currentColor" />
        </button>
        <button onClick={() => setVolume((v) => (v === 0 ? 70 : 0))} aria-label={volume === 0 ? 'Unmute' : 'Mute'} style={{ color: 'var(--ink-faint)', flexShrink: 0, display: 'grid', placeItems: 'center' }}>
          <Icon name={volume === 0 ? 'volume-x' : 'volume-2'} size={16} />
        </button>
        <input type="range" min="0" max="100" value={volume} onChange={(e) => setVolume(Number(e.target.value))}
          aria-label="Volume" className="so-range" style={{ flex: 1, accentColor: 'var(--sage)' }} />
      ...
      {unavailable && (
        <p style={{ fontSize: 11, color: 'var(--ink-faint)', textAlign: 'center', padding: '0 16px 12px', margin: 0 }}>
          Preview audio for this track isn’t bundled — it plays in the full app.
        </p>
      )}
    </div>
  );
...
// ── Safety / support resources page (the /safety route) ─────────────────
// Verified resources only — reused from the project's existing crisis list.
...
function SafetyPage({ onBack, onTryGrounding, onTryBreathing }) {
  const ref = useRefM(null);
  useEffectM(() => { ref.current?.focus(); }, []);
  return (
    <div
      ref={ref} tabIndex={-1} className="fade-in no-scrollbar"
    ...
    >
      <div style={{ maxWidth: 560, margin: '0 auto', padding: '28px 26px 48px' }}>
        <button onClick={onBack} style={{ display: 'inline-flex', alignItems: 'center', gap: 7, fontSize: 13.5, fontWeight: 600, color: 'var(--ink-soft)', marginBottom: 22 }}>
          <Icon name="arrow-left" size={16} /> Back
        </button>
        ...
        <p style={{ fontSize: 14.5, lineHeight: 1.65, color: 'var(--ink-soft)', margin: '0 0 8px' }}>
          SoulMate is a companion for reflection — not a crisis or emergency service. If things feel
          like too much, reaching out to a real person can help more than anything.
        </p>
        <p style={{ fontSize: 13, lineHeight: 1.6, color: 'var(--ink-faint)', margin: '0 0 26px' }}>
          If you’re in immediate danger, please call your local emergency number or go to the nearest
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 30 }}>
          {SAFETY_RESOURCES.map((r) => (
            <a key={r.country} href={`tel:${r.phone.replace(/\s/g, '')}`}
              className="card" style={{
                display: 'flex', alignItems: 'center', gap: 14, padding: '15px 17px', textDecoration: 'none',
                boxShadow: 'var(--shadow-soft)',
                borderColor: r.primary ? 'color-mix(in oklab, var(--sage) 32%, transparent)' : 'var(--line)',
                background: r.primary ? 'var(--sage-tint)' : 'var(--surface)',
              }}>
        ...
        <a href="https://www.iasp.info/resources/Crisis_Centres/" target="_blank" rel="noopener noreferrer"
          style={{ display: 'inline-flex', alignItems: 'center', gap: 7, fontSize: 13, fontWeight: 600, color: 'var(--sage-deep)', marginBottom: 32 }}>
...
Object.assign(window, { GroundingExercise, BreathingModal, CalmingSounds, SafetyPage });
